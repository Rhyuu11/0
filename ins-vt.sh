...
# // Exporting URL Host
export Server_URL="raw.githubusercontent.com/Rhyuu11/test/main"
export Server1_URL="raw.githubusercontent.com/Rhyuu11/limit/main"
...
# Make Folder & Log XRay & Log Trojan-go
rm -fr /var/log/xray
...
wget -q -O /etc/nginx/nginx.conf "https://raw.githubusercontent.com/Rhyuu11/0/main/nginx.conf" 
#mkdir -p /home/vps/public_html
wget -q -O /etc/nginx/conf.d/vps.conf "https://raw.githubusercontent.com/Rhyuu11/0/main/vps.conf"
...
sed -i '$ i{' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_redirect off;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_pass http://127.0.0.1:'"$worri"';' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_http_version 1.1;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header X-Real-IP \$remote_addr;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_set_header Upgrade \$http_upgrade;' /etc/nginx/conf.d/xray.conf
...
rm -fr /etc/systemd/system/trojan-go.service
cat > /etc/systemd/system/trojan-go.service << END
[Unit]
Description=Trojan-Go Service Mod By Akhir Zaman
...
# Trojan Go Uuid
rm -fr /etc/trojan-go/uuid.txt
cat > /etc/trojan-go/uuid.txt << END
$uuid1
END

# Set Iptables For Trojan-Go
systemctl daemon-reload >/dev/null 2>&1
...
echo -e "[ ${green}ok${NC} ] Enable & Start & Restart Trojan-Go & Nginx"
systemctl daemon-reload >/dev/null 2>&1
...
# Enable & Restart Xray & Trojan-Go & Nginx
sleep 1
echo -e "[ ${green}ok${NC} ] Restart Xray & Trojan-Go & Nginx"
systemctl daemon-reload >/dev/null 2>&1
systemctl restart xray >/dev/null 2>&1
systemctl restart trojan-go >/dev/null 2>&1
systemctl restart nginx >/dev/null 2>&1
